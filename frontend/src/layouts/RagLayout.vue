  ...
  <div class="ms-layout">
    <header class="ms-header">
      <div class="ms-header-left">
        ...
        <span class="brand-text">智能诊断</span>
      </div>
...
import {
  DataAnalysis, Reading, Share, ChatDotSquare,
  Document
...
const menuItems = computed(() => {
  const base = [
    { path: '/rag/rag-query', title: '智能问答', icon: ChatDotSquare },
  ]
  if (role.value === 'admin' || role.value === 'data_manager') {
    base.unshift(
      { path: '/rag/dashboard', title: '数据概览', icon: DataAnalysis },
      { path: '/rag/knowledge', title: '知识管理', icon: Reading },
      { path: '/rag/knowledge-graph', title: '知识图谱', icon: Share },
    )
    base.push(
      { path: '/rag/diagnosis', title: '诊断记录', icon: Document },
    )
  } else {
    base.push({ path: '/rag/diagnosis', title: '诊断记录', icon: Document })
  }
  return base
})
...
.brand-text { font-size: 14px; font-weight: 600; color: #1a1a2e; white-space: nowrap; }
...
.ms-tab {
  ...
  font-size: 13px; color: #5f6368; cursor: pointer; white-space: nowrap;
  position: relative; border-radius: 6px 6px 0 0; transition: all .15s;
}
